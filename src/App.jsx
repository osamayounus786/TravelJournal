...
function App() {
  ...
  return (
    <div>
      <h1>Hello {name}</h1>
      <p>This is {obj.name1}</p>
      <p>Showing ['We', 'are', 'United'] in separate tags </p>
      <h2>{data[0]}</h2>
      <h2>{data[1]}</h2>
      <h2>{data[2]}</h2>
      <p>Showing list of array objects in separate tags:</p>
      <p>this is {Object.values(list[0])}</p>
      <p>this is {Object.values(list[1])}</p>
      <p>
        Ali is doing {complex[0].jobs} job in {complex[0].company}
      </p>
      <p>
        Sameer is doing {complex[1].jobs} job in {complex[1].company}
      </p>
...
However it is up to you to choose how to export things: for example, a module might have no default export at all.`}</pre>
    
    { /* Importing user from user */ }
    {post.map((data) => {
          return (
            <div>
